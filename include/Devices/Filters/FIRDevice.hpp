...
class FIRDevice : virtual public SingleInputDevice<cv::Point2f, cv::Point2f>
{
    private:
        ...
        void update_coeff(int halfLength, WindowType w_type)
        {
            ...
            for (int i = 0; i < TAPS; i++) { coeff[i] *= normalization; }
        }
    ...
    public:
        ...
        FIRDevice(double sampFreq, double transFreq, FilterType f_type, WindowType w_type) :
            SingleInputDevice<cv::Point2f, cv::Point2f>::SingleInputDevice(cv::Point2f(0.0, 0.0)), offset(0), buffer(nullptr)
        {
            ...
            if (0.5 <= ft) { throw std::invalid_argument("The transition frequency must be lesser than the Nyquist frequency"); }
